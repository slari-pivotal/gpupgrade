...
var _ = Describe("Hub check config", func() {
	...
	It("successfully writes config for GPDB 6", func() {
		...
		newClusterPair, err := services.SaveOldClusterConfig(clusterPair, dbConnector, dir, oldBinDir)
		Expect(err).ToNot(HaveOccurred())
	...
	It("fails to write file", func() {
		...
		_, err := services.SaveOldClusterConfig(clusterPair, dbConnector, dir, oldBinDir)

...
func TestClonePortsFromCluster(t *testing.T) {
	...
	for _, c := range errorCases {
		t.Run(fmt.Sprintf("returns an error %s", c.name), func(t *testing.T) {
			...
			// check() verifies that the error is the one we expect.
			c.check(t, err)
		})
	}
}

// match is a helper for the errorCases table test above. You can use it as the
// errorCase.check() callback implementation; it just uses errors.Is() to see
// whether the actual error returned matches the expected one, and complains
// through the testing.T if not.
func match(expected error) func(*testing.T, error) {
